...
const (
	SelectTransactionList         = `SELECT id, employee_id, room_id, description, status, created_at, updated_at FROM transactions ORDER BY created_at DESC LIMIT $1 OFFSET $2`
	...
	UpdatePermission = `UPDATE transactions SET status = $1, updated_at = $3 WHERE id = $2 RETURNING employee_id, room_id, description, created_at`
